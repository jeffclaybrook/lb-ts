export const MenuItems = [
    {
		name: "Baba Ganoush",
		description: "Spicy, aromatic roasted eggplant mixed with olive oil, tahini salsa, fresh lemon juice, and family spice recipe.",
		price: "$6.99",
		category: "Appetizers",
		image: "./images/appetizer-1.webp"
	},
	{
		name: "Falafel",
		description: "8 deep-fried balls made from ground chickpeas, assorted green vegetables and spices.",
		price: "$6.99",
		category: "Appetizers",
		image: "./images/appetizer-2.webp"
	},
	{
		name: "Hummus Joy",
		description: "Delicious, creamy, and flavorful hummus drizzled with Lebanese olive oil.",
		price: "$6.99",
		category: "Appetizers",
		image: "./images/appetizer-3.webp"
	},
	{
		name: "Cucumber Dill Dip",
		description: "Savory plain yogurt dip/salad made with fresh cucumber, fresh garlic, and dill.",
		price: "$7.99",
		category: "Appetizers",
		image: "./images/appetizer-4.webp"
	},
	{
		name: "Dolmeh Grape Leaves",
		description: "Chopped beef, rice, onions, and aromatic spices hand-wrapped and slowly cooked for a melt-in-your-mouth texture.",
		price: "$9.00",
		category: "Appetizers",
		image: "./images/appetizer-5.webp"
	},
	{
		name: "Middle East Salad",
		description: "Diced tomatoes, onions, cucumber, and peppers tossed with a simple vinaigrette of lemon and olive oil.",
		price: "$5.99",
		category: "Appetizers",
		image: "./images/appetizer-6.webp"
	},
	{
		name: "Mixed Pickle Vegetables",
		description: "Pickled vegetables with turmeric and spices mix.",
		price: "$3.99",
		category: "Appetizers",
		image: "./images/appetizer-7.webp"
	},
	{
		name: "Tangy Pickled Turnips",
		description: "Vinegary pickled turnips with a pinch of heat. The perfect complement to any of the savory dishes.",
		price: "$3.99",
		category: "Appetizers",
		image: "./images/appetizer-8.webp"
	},
	{
		name: "Mama Ganoush",
		description: "A sumptuous and hearty dish, meaty with no meat kind of vibes, eggplant dip made with roasted smoky eggplant, tomatoes, onion, garlic, olive oil, lemon juice, parsley, tomato paste, and salt.",
		price: "$6.99",
		category: "Appetizers",
		image: "./images/appetizer-9.webp"
	},
	{
		name: "Home Fries",
		description: "Herbed and flavored home fries packed with extreme flavor with our famous Tzatziki salsa.",
		price: "$5.99",
		category: "Appetizers",
		image: "./images/appetizer-10.webp"
	},
	{
		name: "Tabbouleh Salad",
		description: "Our tasty tabbouleh salad consists of bulgur, tomatoes, a blend of herbs, and lemon juice.",
		price: "$5.99",
		category: "Appetizers",
		image: "./images/appetizer-11.webp"
	},
	{
		name: "Gyro Beef with Basmati Rice",
		description: "Flour, ground beef, and spices deep fried and served with our house salad, in-house hummus, and white Jasmine rice.",
		price: "$12.99",
		category: "Dishes",
		image: "./images/dish-1.webp"
	},
	{
		name: "Chicken Shawarma Plate",
		description: "Spicy marinated chicken strips served with fresh salad and royal rice served with fresh naan bread.",
		price: "$12.00",
		category: "Dishes",
		image: "./images/dish-2.webp"
	},
	{
		name: "Falafel Plate",
		description: "Garbanzo beans, greens, and spices is served on the top layer of rice, along with veggies, a side dipping hummus, and creamy tahini sauce.",
		price: "$10.00",
		category: "Dishes",
		image: "./images/dish-3.webp"
	},
	{
		name: "Chicken Curry with Rice",
		description: "Try out our tasty chicken curry with a side of rice.",
		price: "$12.99",
		category: "Dishes",
		image: "./images/dish-4.webp"
	},
	{
		name: "Beef Shawarma Plate",
		description: "Tender beef mixed with homemade spices and roasted onion and green pepper served with hummus, Middle Eastern salad, and white rice.",
		price: "$12.99",
		category: "Dishes",
		image: "./images/dish-5.webp"
	},
	{
		name: "Beef Shawarma Sandwich",
		description: "Beef shawarma with pita bread, parsley, onions, tomatoes, radishes, tahini sauce, and beef.",
		price: "$12.00",
		category: "Dishes",
		image: "./images/dish-6.webp"
	},
	{
		name: "Vegetarian Shawarma with Rice",
		description: "Crunchy mix of different cuts of fresh vegetables, tossed with aromatic spices and stir-fried to perfection and served with Basmati rice.",
		price: "$10.99",
		category: "Dishes",
		image: "./images/dish-7.webp"
	},
	{
		name: "Fried Rice mix with Rosted Lamb Iraqi Style",
		description: "Basmati rice blended with an assortment of vegetables and a unique mix of biryani baharat and topped off with a gorgeous roasted piece of lamb.",
		price: "$15.00",
		category: "Dishes",
		image: "./images/dish-8.webp"
	},
	{
		name: "Navy Beans mixed with Lamb and Rice",
		description: "Cannellini beans along with lamb chops cooked to perfection and served with white rice, bread, and salad.",
		price: "$13.99",
		category: "Dishes",
		image: "./images/dish-9.webp"
	},
	{
		name: "Fresh Okra with Lamb Meat Stew and Rice",
		description: "Aromatic, lemony, and flavored dish, tender Lamb mixed with fresh Okra, tomato paste, and fresh lemon and served with rice.",
		price: "$15.00",
		category: "Dishes",
		image: "./images/dish-10.webp"
	},
	{
		name: "Lentil Soup with French Cumin Seeds",
		description: "Lentil soup consisting of carrots, fried onion, turmeric and house spice blend slow boiled to perfection.",
		price: "$7.99",
		category: "Dishes",
		image: "./images/dish-11.webp"
	},
	{
		name: "Falafel Wrap",
		description: "One of the most popular Middle Eastern street foods, it's super flavorful and soft on the inside, but crunchy on the outside.",
		price: "$9.99",
		category: "Dishes",
		image: "./images/dish-12.webp"
	},
	{
		name: "Fried Kibbeh",
		description: "Crispy golden fried basmati rice filled with roasted pine nuts, caramelized onion, Raisin, and ground beef.",
		price: "$9.99",
		category: "Dishes",
		image: "./images/dish-13.webp"
	},
	{
		name: "Vegetable Wrap",
		description: "Sautéed vegetables and hummus, Arabic style pickles wrapped in Pita bread Make a tasty and deliciously tangy one-dish meal!",
		price: "$9.99",
		category: "Dishes",
		image: "./images/dish-14.webp"
	},
	{
		name: "Vegan lentil Soup with Fresh Cumin Seeds and Turmeric",
		description: "One of your iconic and highly loved lentil soup consisting of carrots, fried onion, turmeric and house spice blend slow boiled to perfection.",
		price: "$5.99",
		category: "Dishes",
		image: "./images/dish-15.webp"
	},
	{
		name: "Foul Medames",
		description: "Simple and delicious Egyptian comfort food is usually served with pita. It is made of fava beans, freshly chopped vegetables, and spices. It is tangy and spicy with different textures of vegetables and some chunks of unmashed beans creating a perfect texture of smoothness and crunch.",
		price: "$12.99",
		category: "Dishes",
		image: "./images/dish-16.webp"
	},
	{
		name: "Baklava",
		description: "Made from layers of golden brown phyllo filled with chopped walnuts, sweeten with honey syrup, and a top with crushed pistachio.",
		price: "$4.00",
		category: "Desserts",
		image: "./images/dessert-1.webp"
	},
	{
		name: "Sopapilla Cheesecake",
		description: "Flaky crust with sweet cream cheese filling topped with cinnamon sugar.",
		price: "$4.00",
		category: "Desserts",
		image: "./images/dessert-2.webp"
	},
	{
		name: "Falafel with Eggs",
		description: "Mixing falafel with eggs is a great idea for a breakfast dish that combines the protein and fiber from the falafel with the nutrients and richness of eggs. Comes with pita bread and salad with hummus.",
		price: "$12.99",
		category: "Breakfast",
		image: "./images/breakfast-1.webp"
	},
	{
		name: "Beef Gyro with Eggs",
		description: "This hearty meal typically includes a beef gyro with seasoned potatoes, a combination of protein, carbohydrates, and fats that can keep you fueled throughout the day. Comes with pita bread and salad with hummus.",
		price: "$14.99",
		category: "Breakfast",
		image: "./images/breakfast-2.webp"
	},
	{
		name: "Chicken Shawarma with Eggs",
		description: "Chicken with eggs and seasoned potatoes is a delicious breakfast. Chicken shawarma and eggs are both good sources of protein, which can help you feel fuller for longer and provide energy throughout the day. Comes with pita bread and salad with hummus.",
		price: "$13.99",
		category: "Breakfast",
		image: "./images/breakfast-3.webp"
	},
	{
		name: "Stir Fry Tomato with Eggs and Group Beef",
		description: "This is one of the most popular Iraqi breakfast dishes. Tomatoes are stir-fried with egg, and spring onions with ground beef. Best served with a side of rice or naan bread. Comes with pita bread and salad with hummus.",
		price: "$14.99",
		category: "Breakfast",
		image: "./images/breakfast-4.webp"
	},
	{
		name: "Lahm Bajeen (1 Piece)",
		description: "Grilled sour and sweet ground beef lays on homemade tortilla bread. This recipe was handed down by my mother, and it is a family favorite. The mix of the fresh tortilla and the ground beef flavored with my homemade spices makes this meal your favorite snack. Comes with pita bread and salad with hummus.",
		price: "6.99",
		category: "Breakfast",
		image: "./images/breakfast-5.webp"
	},
	{
		name: "Eggs & potatoes over beef shawarma plate",
		description: "Eggs and potatoes are a nice combination for a hearty breakfast. This recipe features spicy home fries with classic scrambled eggs over our famous shawarma beef.",
		price: "15.99",
		category: "Breakfast",
		image: "./images/breakfast-6.webp"
	},
	{
		name: "Hibiscus Tea",
		description: "Popular herbal street drink made from dehydrated hibiscus with a sweet and sour taste.",
		price: "$2.00",
		category: "Drinks",
		image: "./images/drink-1.webp"
	},
	{
		name: "Lemonade Refreshing Iced Tea",
		description: "Fresh cardamom and lemon juice. It is sweet & delightful drink with no artificial add-ons.",
		price: "$2.00",
		category: "Drinks",
		image: "./images/drink-2.webp"
	},
	{
		name: "Rose & Pomegranate Tea",
		description: "Rose flavored and sweet Pomegranate cold beverage.",
		price: "$2.00",
		category: "Drinks",
		image: "./images/drink-3.webp"
	},
	{
		name: "Chill Tamarind Drink",
		description: "It's a cool, sweet, and tangy drink with an awesome, refreshing taste. It really is a cool drink for a scorching summer",
		price: "$1.99",
		category: "Drinks",
		image: "./images/drink-4.webp"
	}
]